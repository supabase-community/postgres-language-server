...
test.describe("Monaco Editor with PGLS Language Server (Web Worker)", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForFunction(
      () => {
        ...
        return status?.classList.contains("ready") || status?.classList.contains("error");
      },
      { timeout: 30000 },
    );

    // Check if WASM loaded successfully
    const hasError = await page.locator("#status").evaluate((el) => el.classList.contains("error"));
  ...
  test("setSchema via LSP notification enables schema-aware completions", async ({ page }) => {
