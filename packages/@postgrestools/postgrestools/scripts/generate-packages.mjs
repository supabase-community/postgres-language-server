...
async function downloadSchema(releaseTag, githubToken) {
  ...
  // download to root.
  const fileStream = fs.createWriteStream(resolve(POSTGRESTOOLS_ROOT, "schema.json"));
...
async function writeManifest(packagePath, version) {
  const manifestPath = resolve(PACKAGES_POSTGRESTOOLS_ROOT, packagePath, "package.json");

  const manifestData = JSON.parse(fs.readFileSync(manifestPath).toString("utf-8"));

  const nativePackages = platformArchCombinations().map(({ platform, arch }) => [
