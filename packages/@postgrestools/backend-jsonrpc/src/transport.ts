...
export class Transport {
   ...
   */
  // oxlint-disable-next-line typescript/no-explicit-any -- if i change it to Promise<unknown> typescript breaks
    ...
    if (isJsonRpcMessage(body)) {
      ...
      if (isJsonRpcResponse(body)) {
        ...
        if (pendingRequest) {
          ...
          throw new Error(`could not find any pending request matching RPC response ID ${body.id}`);
        }
        return;
      }
    }
