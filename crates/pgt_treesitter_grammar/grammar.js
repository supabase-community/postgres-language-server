  ...
  rules: {
    ...
    array: ($) =>
      seq(
        $.keyword_array,
        choice(
          seq("[", comma_list($._expression, false), "]"),
          seq("(", $._dml_read, ")")
        )
      ),
    ...
    policy_to_role: ($) =>
      seq($.keyword_to, comma_list($.role_specification, true)),
          ...
          seq(
            ...
            choice(
              ...
              seq(
                ...
                choice($.any_identifier, $.keyword_default)
              ),
              seq($.keyword_role, choice($.role_identifier, $.keyword_none))
            )
          ),
          seq(
            $.keyword_constraints,
        ...
        choice(
          ...
          seq(
            ...
            choice(
              ...
              seq($.keyword_compression, field("compression_method", $._word)),
    ...
    change_column: ($) =>
      seq(
        ...
        optional($.keyword_column),
        optional($._if_exists),
        field("old_name", $.column_identifier),
        $.column_definition,
        optional($.column_position)
      ),
    ...
    column_position: ($) =>
      choice(
        $.keyword_first,
        seq($.keyword_after, field("col_name", $.column_identifier))
      ),
    ...
    set_schema: ($) =>
      seq(
        $.keyword_set,
        $.keyword_schema,
        field("schema", $.schema_identifier)
      ),

    change_ownership: ($) =>
      seq($.keyword_owner, $.keyword_to, $.role_identifier),
    ...
    table_option: ($) =>
      choice(
        ...
        seq(
          field(
            "name",
            choice($.keyword_engine, $.any_identifier, $._literal_string)
          ),
          "=",
    ...
    index_hint: ($) =>
      seq(
        ...
        wrapped_in_parenthesis(field("index_name", $.any_identifier))
      ),
    ...
    any_identifier: ($) => $._identifier,
    column_identifier: ($) => $._identifier,
    table_identifier: ($) => $._identifier,
    schema_identifier: ($) => $._identifier,
    role_identifier: ($) => $._identifier,
    policy_identifier: ($) => $._identifier,
 ...
 * @returns {ChoiceRule}
 */
function unsigned_type($, rule) {
  return choice(
    seq($.keyword_unsigned, rule),
    seq(rule, optional($.keyword_unsigned), optional($.keyword_zerofill))
  );
}
 ...
 * @returns {SeqRule | ChoiceRule}
 */
function comma_list(rule, requireFirst) {
  let sequence = seq(rule, repeat(seq(",", rule)));
