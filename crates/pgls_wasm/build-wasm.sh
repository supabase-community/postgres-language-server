...
# The cc crate needs these to use emcc instead of clang
...
# The build produces a .wasm file directly when using wasm32-unknown-emscripten target
WASM_FILE="$OUT_DIR/pgls_wasm.wasm"
...
cp "$WASM_FILE" "$DIST_DIR/pgls.wasm"

# Generate JS wrapper for the WASM module with Emscripten-compatible interface
# This provides both low-level FFI functions and Emscripten runtime methods
cat > "$DIST_DIR/pgls.js" << 'WRAPPER_EOF'
...
echo "Build complete!"
echo "Output files:"
echo "  $DIST_DIR/pgls.js"
echo "  $DIST_DIR/pgls.wasm"
echo ""
