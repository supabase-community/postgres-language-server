...
pub fn find_option_value(
    create_fn: &pgt_query::protobuf::CreateFunctionStmt,
    option_name: &str,
) -> Option<String> {
    create_fn
        .options
        ...
        .find_map(|opt| {
            if let pgt_query::NodeEnum::DefElem(def_elem) = opt {
                if def_elem.defname == option_name {
                    def_elem
                        ...
                        .find_map(|arg| {
                            if let pgt_query::NodeEnum::String(s) = arg {
                                Some(s.sval.clone())
                            } else if let pgt_query::NodeEnum::List(l) = arg {
                                l.items.iter().find_map(|item_wrapper| {
                                    if let Some(pgt_query::NodeEnum::String(s)) =
...
pub fn parse_name(nodes: &[pgt_query::protobuf::Node]) -> Option<(Option<String>, String)> {
    let names = nodes
        .iter()
        .map(|n| match &n.node {
