-- meta: name = policyExistsRlsDisabled
-- meta: title = Policy Exists RLS Disabled
-- meta: severity = ERROR
-- meta: category = SECURITY
-- meta: description = Detects cases where row level security (RLS) policies have been created, but RLS has not been enabled for the underlying table.
-- meta: remediation = https://supabase.com/docs/guides/database/database-linter?lint=0007_policy_exists_rls_disabled
