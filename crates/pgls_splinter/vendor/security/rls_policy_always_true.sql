-- meta: name = rlsPolicyAlwaysTrue
-- meta: title = RLS Policy Always True
-- meta: severity = WARN
-- meta: category = SECURITY
-- meta: description = Detects RLS policies that use overly permissive expressions like USING (true) or WITH CHECK (true) for UPDATE, DELETE, or INSERT operations. SELECT policies with USING (true) are intentionally excluded as this pattern is often used deliberately for public read access.
-- meta: remediation = https://supabase.com/docs/guides/database/database-linter?lint=0024_permissive_rls_policy
