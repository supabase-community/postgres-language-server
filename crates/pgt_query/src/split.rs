...
/// let statements = pgt_query::split_with_parser(query).unwrap();
...
/// let result = pgt_query::split_with_parser(query);
/// let err = r#"syntax error at or near "this""#;
/// assert_eq!(result, Err(pgt_query::Error::Split(err.to_string())));
/// ```
...
/// let statements = pgt_query::split_with_scanner(query).unwrap();
