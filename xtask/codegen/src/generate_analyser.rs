...
fn update_linter_registry_builder(
    ...
    // Generate factory match arms for all rules
    let factory_arms = all_rules.iter().map(|(rule_name, (rule_path, _))| {
        quote! {
            #rule_name => Some(Box::new(|| crate::linter_registry::RegistryLinterRule::new::<#rule_path>()))
        }
    });
    ...
    let tokens = xtask::reformat(quote! {
        ...
        /// Maps rule keys to factory functions that create rule executors
        /// This function is generated by codegen and includes all linter rules
        pub fn get_linter_rule_factory(
            key: &RuleKey,
        ) -> Option<Box<dyn FnOnce() -> RegistryLinterRule>> {
            match key.rule_name() {
                #( #factory_arms, )*
                _ => None,
            }
